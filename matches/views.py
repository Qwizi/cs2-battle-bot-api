...
class MatchViewSet(viewsets.ModelViewSet):
    ...
    def __divide_players(self, players_list: list[Player]) -> tuple[list[Player], list[Player]]:
        """
    ...
    def __create_default_teams(self, team1_name: str, team2_name: str, players_list: list[Player]) -> tuple[Team, Team]:
        """
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            map_sides = serializer.validated_data.get("map_sides", ["knife", "team1_ct", "team2_ct"])
            ...
            try:
                ...
                if shuffle_players:
                    ...
                    team1_players = {str(player.steam_user.steamid64): player.steam_user.username for player in team1.players.all()}
                    team2_players = {str(player.steam_user.steamid64): player.steam_user.username for player in team2.players.all()}
                else:
                    if not team1_id or not team2_id:
                        return Response({"message": "team1_id and team2_id ids are required"}, status=400)
                    try:
                        team1 = Team.objects.get(id=team1_id)
                        team2 = Team.objects.get(id=team2_id)
                        players_ids = [player.id for player in players_list]
                        team1_players = {str(player.steam_user.steamid64): player.steam_user.username for player in team1.players.filter(pk__in=players_ids)}
                        team2_players = {str(player.steam_user.steamid64): player.steam_user.username for player in team2.players.filter(pk__in=players_ids)}
                    except Team.DoesNotExist as e:
                        return Response({"message": f"Team {e} not exists"}, status=400)
                for map in maplist:
                    if map not in Map.objects.filter().values_list("tag", flat=True):
                        return Response({"message": f"Map {map} not exists"}, status=400)
