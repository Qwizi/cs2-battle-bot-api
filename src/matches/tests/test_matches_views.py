...
from cs2_battle_bot.tests.conftest import api_client, client_with_api_key, client_with_token
from matches.models import Match, MatchType, MatchStatus, Map
...
@pytest.mark.parametrize("map_sides", [["knife", "knife", "knife"], ["knife", "team2_ct", "team1_t"]])
@pytest.mark.parametrize("cvars", [{"sv_cheats": "1"}])
def test_create_match(client_with_api_key, match_data, with_server, match_type, server, mocker, map_sides,
                      clinch_series, cvars):
...
def test_create_match_with_not_available_server(client_with_api_key, match_with_server, match_data, server,
                                                match_status, mocker):
    ...
    response = client_with_api_key.post(API_ENDPOINT, match_data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.data["message"] == "Server is not available for a match. Another match is already running"


@pytest.mark.django_db
@pytest.mark.parametrize("with_server", [True, False])
def test_create_match_with_invalid_discord_users(client_with_api_key, match_data, with_server, server, mocker):
...
@pytest.mark.django_db
def test_create_match_with_not_enough_discord_users(client_with_api_key, match_data, players):
...
def test_match_ban_map(client_with_api_key, match, match_type):
    ...
    assert response.data["next_ban_team"]["leader"]["discord_user"][
...
def test_match_pick_map(client_with_api_key, match, match_type):
    ...
    team1_ban_response = client_with_api_key.post(f"{API_ENDPOINT}{match.pk}/ban/", team1_ban_data)
    assert team1_ban_response.status_code == status.HTTP_200_OK
    team2_ban_data = {
        "interaction_user_id": team2_leader.discord_user.user_id,
        "map_tag": "de_overpass"
    }
    team2_ban_response = client_with_api_key.post(f"{API_ENDPOINT}{match.pk}/ban/", team2_ban_data)
...
def test_match_b01_map_veto_flow(client_with_api_key, match):
    ...
    assert t1_ban_1_response.data["next_ban_team"]["leader"]["discord_user"][
    ...
    assert t2_ban_1_response.data["next_ban_team"]["leader"]["discord_user"][
    ...
    assert t1_ban_3_response.data["next_ban_team"]["leader"]["discord_user"][
               "user_id"] == team2_leader.discord_user.user_id
    ...
    assert t2_ban_3_response.data["next_ban_team"]["leader"]["discord_user"][
               "user_id"] == team1_leader.discord_user.user_id
...
def test_match_ban_map_with_first_team2(client_with_api_key, match, match_type):
    ...
    data = {
        "interaction_user_id": team2_leader,
        "map_tag": "de_mirage"
    }
...
def test_match_ban_map_with_already_banned_map(client_with_api_key, match, match_type):
    ...
    data = {
        "interaction_user_id": team_leader,
        "map_tag": "de_mirage"
    }
...
def test_match_shuffle(client_with_api_key, match):
    response = client_with_api_key.post(f"{API_ENDPOINT}{match.pk}/shuffle/", data={
        "interaction_user_id": match.author.user_id
    })
    assert response.status_code == status.HTTP_200_OK
...
def test_match_shuffle_with_invalid_author(client_with_api_key, match):
    response = client_with_api_key.post(f"{API_ENDPOINT}{match.pk}/shuffle/", data={
        "interaction_user_id": match.team2.leader.discord_user.user_id
    })
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_match_recreate(client_with_api_key, match, match_with_server, with_server):
    match_to_test = match if not with_server else match_with_server
    response = client_with_api_key.post(f"{API_ENDPOINT}{match.pk}/recreate/", data={
        "interaction_user_id": match.author.user_id
    })
    assert response.status_code == status.HTTP_201_CREATED
    assert response.data["status"] == MatchStatus.CREATED
